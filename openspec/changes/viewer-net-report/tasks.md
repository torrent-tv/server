...
- [ ] 1.1 Rolling sample store: per-fetch `{ bytes, ms, at }` from the
      existing dc-load instrumentation, ~30 s window; median Mbit/s
      accessor (transfers only, ignore cache/zero-ms samples).
- [ ] 1.2 Reporter: 10 s interval tied to the transcode-session lifecycle
...
- [ ] 1.3 Preview: module loads clean; reporter no-ops without a session.
