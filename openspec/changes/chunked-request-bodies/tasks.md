...
- [ ] 1.1 Constants; frame builder helper (mirrors `#onResponseBinaryChunk`
      layout; bit 0 done, bit 1 aborted); set
      `bufferedAmountLowThreshold = REQUEST_BUFFERED_LOW_BYTES` at channel
      creation. `node --check`.
- [ ] 1.2 `fetch()`: byte-measured body path decision (threshold), chunked
      writer with backpressure, abort frame on signal, unchanged
      pending-entry lifecycle. `node --check`.
...
- [ ] 2.1 Unit (node): stub channel object capturing `send()` calls with a
      ...
      the body; threshold body → single legacy message; abort mid-send →
      abort frame + AbortError.
- [ ] 2.2 Preview: app boots with zero console errors; a normal (small
      magnet) flow against the dev HA proxy is unchanged.
- [ ] 2.3 E2E with the proxy on 2.9.35 (local stack or dev HA): register
      the real Poirot `.torrent` via the UI — plan returns, playlist
      appears, an episode reaches the transcode phase; proxy log shows
      `[dc] … body=… (chunked)`.

## 3. Release

- [ ] 3.1 CHANGELOG at current version + 1 patch (expected 0.8.58);
      `npm run patch` AFTER proxy 2.9.35 is live in HA; verify via
      `window.env.version` and a live Poirot registration.
